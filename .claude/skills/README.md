<<<<<<< HEAD
# Nepally Skills Reference
=======
# NUSA Skills Reference
>>>>>>> origin/master

## Available Skills

| Skill | Purpose |
|-------|---------|
| `/design-feature [name]` | Create feature specification |
| `/user-journey [name]` | Document user flow |
| `/wireframe [name]` | Create screen wireframe (WireMD format) |
| `/implement-feature [name]` | Implement feature with full context gathering |
| `/shared-first-check` | Audit shared-first architecture compliance |
| `/break-features [phase]` | Break roadmap phase into small features |
| `/refine-roadmap [section]` | Analyze and improve roadmap sections |

## Recommended Workflow

1. `/break-features` — Break phase into features
2. `/design-feature` — Spec out individual features
3. `/user-journey` — Document user flows
4. `/wireframe` — Design screens
5. `/implement-feature` — Build it (reads all docs automatically)
6. `/shared-first-check` — Validate architecture compliance
7. `/refine-roadmap` — Improve roadmap based on learnings
