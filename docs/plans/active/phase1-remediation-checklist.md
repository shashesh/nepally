...
| MOD-03   | P0       | Moderation    | Moderator queue: pending (Emergency) post approvals, report review, ban/unban; auto-hide at 3 reports                                                                                                                                                                                                                                           | TBD   | Completed   | Sprint 1      | 2026-09-04   | Migration 035 + `/moderation` web page + `npm run test:security:emergency-post`. Also fixed: report inserts failed RLS for non-moderators (RETURNING needs SELECT policy)                                                                                                                                                       |
| SEC-02   | P0       | Security      | Block self-escalation of `users` privileged columns (trust_level / is_moderator / is_premium)                                                                                                                                                                                                                                                   | TBD   | Completed   | Sprint 1      | 2026-09-04   | Migration 034 guard trigger + `mark_user_verified()` RPC; `npm run test:security:users-privilege`                                                                                                                                                                                                                               |
| SEC-03   | P0       | Security      | Stop `users` PII (email / phone / zip_code / ban_reason) being readable by anyone with the anon key                                                                                                                                                                                                                                             | TBD   | Completed   | Sprint 1      | 2026-09-08   | Migration 036 column-level SELECT grant + `get_my_profile()` RPC; shared `getUserById` -> `PublicUser`, new `getMyProfile`; `npm run test:security:users-pii`                                                                                                                                                                   |
| SEC-04   | P0       | Security      | Block conversation self-join (any user could insert themselves into `conversation_participants` and read the thread)                                                                                                                                                                                                                            | TBD   | Completed   | Sprint 1      | 2026-09-08   | Migration 036 policy requires creator or existing participant (`is_conversation_creator()`); SEC-01 smoke test had no self-join case — `chat-rls-smoke.ts` now covers it                                                                                                                                                        |
| SEC-05   | P0       | Security      | Require service-role caller on `send-push-notification` (verify_jwt accepted the public anon key; anyone could push spoofed titles/URLs to any user)                                                                                                                                                                                            | TBD   | Completed   | Sprint 1      | 2026-09-08   | Edge function constant-time compares the bearer token to `SUPABASE_SERVICE_ROLE_KEY`; DB trigger (009/012) already sends it. Deployed to nusa-staging (v4, verify_jwt now true) 2026-09-08; live-verified: no header -> gateway 401, anon key -> function 401                                                                   |
| PAY-01   | P0       | Payments      | Stripe webhook used sync `constructEvent`, which throws under Deno's async-only SubtleCrypto — deliveries rejected as invalid signature, promotions never activated                                                                                                                                                                             | TBD   | Completed   | Sprint 1      | 2026-09-08   | Switched to `constructEventAsync`. Deployed to nusa-staging (v3) 2026-09-08; live-verified signature check runs (400 Invalid signature on a bad sig, not a crash). Still check the Stripe dashboard delivery log for promotions stuck pending from prior 400s                                                                   |
| SEC-06   | P1       | Security      | Hardening backlog from 2026-09-08 design review: bucket `allowed_mime_types` + `file_size_limit` (avatars/post-photos/listing-photos); shared-secret header on `expire-posts` / `expire-promotions`; mobile session to `expo-secure-store`; confirm prod `enable_confirmations` (trust ladder is a no-op if off); pgTAP negative-case RLS tests | TBD   | In Progress | Sprint 3      | 2026-09-18   | Not exploitable today; tracked so it is not lost. Mobile session now encrypted at rest (AES key in `expo-secure-store`, existing sessions migrated). Bucket limits (045) and promotion expiry on pg_cron (046, replacing both cron edge functions) done on staging 2026-09-25; prod `enable_confirmations` and pgTAP still open |
