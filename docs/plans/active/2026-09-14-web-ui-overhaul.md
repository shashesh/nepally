...
**The promotion success page is not in either.** Its stranding bug — every poll failing left a member who had already paid with no link, no error and no retry — was found during this recon and fixed on its own branch as PR #87, so it reaches `master` without waiting for the migration. 8b still owns its visual migration.
...
| `packages/shared/src/logic/marketplace/seller.ts` (+ test) | Create | `isVerifiedSeller`, today copied three times |
...
One view behind both routes (decision 1). It renders `FilterBar`, the three strips when unfiltered, the grid, and `LoadingState` / `EmptyState` / `ErrorState`. The pages become thin: each parses its query, resolves its metro, and renders the view with the right heading. `<Link><Button></Link>` at index 237–242 and 321–323 becomes `<Button component={Link} href=…>`.
...
- **Cleanup:**
  ...
  - [ ] **Paging offsets count unique rows, not consumed rows.** `useMarketplaceFeed` asks for the next page at `grid.length`, but `appendPage` drops ids already on screen, so a duplicate makes the offset undercount the real cursor and the next request can re-fetch the same window while `hasMore` stays true. The pre-chunk pages had the identical bug, and `getListingsByMetro`'s sorts are not tie-broken by id, so concurrent inserts can produce one. Decide between a row-consumed cursor and tie-breaking the sort (found in PR 8a's chunk 2 review).
