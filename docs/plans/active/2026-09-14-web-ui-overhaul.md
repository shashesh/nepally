...
- **Cleanup:**
  ...
  - [ ] Search's result `Tabs` (`search.page.tsx:125`) wrap on phones like the public profile's did. Give them the same `tabList`/`tab` treatment (nowrap + scroll, `flex-shrink: 0`, inset focus ring, token underline) (found in PR 6's Task 6.10 review).
