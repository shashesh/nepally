...
| 8a Marketplace: browse + listing detail | `feat/web-ui-marketplace` | 8a.1–8a.12, in four chunks | In Progress | 2026-09-22 | branched from `master` at `aef9fca`. Owns both visual pages, so it takes `a11y-baseline.json` to `{}`. Recon and breakdown done 2026-09-22; chunk 1 not started |
...
The raw `<input role="searchbox">` (107–116) and its emoji `searchIcon` become a Mantine `TextInput` with `leftSection={<IconSearch aria-hidden />}` and a `CloseButton` in `rightSection` that clears the field — the treatment `EventFilterBar` already established, which this control has never had. The `Select` stays as it is. `outline: none` at `.module.css` 38 goes, and the field takes the theme focus ring.
