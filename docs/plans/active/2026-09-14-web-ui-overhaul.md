...
1. **Apply migration 040** (`users_full_name_length_check`) to `nusa-staging`. 10c wrote it and made `createUserProfile` fit it; applying it needs the user (the session's permission check stopped the agent). Before applying, re-check `max(char_length(full_name))`, which was 17 on 2026-09-24. Afterwards, read the constraint back from `pg_constraint`.
