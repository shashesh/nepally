...
- **Never commit on `master`.** Every PR starts with `git switch master && git pull --ff-only && git switch -c <branch>`. Commit freely. **`git push` needs the user's explicit approval**: show the branch, commits and `git diff --stat master...HEAD` first.
...
- Database: the only new migration is `supabase/migrations/037_search.sql`. It is additive only; never touch `001`–`003`. Apply it with the Supabase MCP `apply_migration` tool or the dashboard SQL editor, never `supabase db push`.
...
1. **No Docker locally.** Screenshot baselines are generated by the Visual baselines workflow. That workflow also runs on pushes to non-`master` branches whose head commit message contains `[visual-baselines]`. PRs are stacked branches, each pushed with the user's approval.
