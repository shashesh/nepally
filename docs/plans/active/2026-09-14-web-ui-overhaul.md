...
18. `createUserProfile` cuts a provider name at 100 code points, which never splits a surrogate pair but can split a ZWJ emoji sequence at the boundary. `Intl.Segmenter` would cut on grapheme boundaries.
19. `fullNameSchema.max(100)` counts UTF-16 units while 040's `char_length` counts code points, so the form is stricter than the database for names with emoji. Harmless, but the two limits don't match.

**Product decisions still open**

20. What a paid promotion should do when its listing is inactive at payment time or is deactivated mid-run (a webhook check before activation, a refund, a pause) — from PR 8b.
