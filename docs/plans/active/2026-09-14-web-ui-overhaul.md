...
- **Cleanup:**
  ...
  - [ ] **A reused realtime topic can leave a thread without live updates.** Shared `subscribeToMessages` names its channel `messages:<conversationId>`, and realtime-js's `client.channel(topic)` returns an existing channel with that topic. Going A → B → A while A's leave is still pending (e.g. offline, where the leave waits for its timeout) attaches the new bindings to the leaving channel, and `.subscribe()` does nothing. Pre-existing; a unique topic suffix per subscription would fix it for web and mobile (found in PR 9a's chunk 1 review).
