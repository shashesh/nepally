...
- [ ] **Step 1: Run the smoke pass**, which needs no Docker: `node scripts/visual/smoke.mjs`.
- [ ] **Step 2: Run the e2e suite.** `npm run test:e2e:web`. The known risks are the period-aware mock (Task 7.10), the renamed buttons, and `11-marketplace.spec.ts`, which shares `supabase-mock.ts`.
- [ ] **Step 3: Check the definition of done's structural items.**
  ...
  - The page line counts are near their targets.
- [ ] **Step 4: Regenerate the baselines** locally in Docker, as PR 6 did, with `npm run test:visual:docker --workspace=apps/web -- --update --write-a11y-baseline`. Review **every** changed PNG against the list above, and commit the screenshots and the a11y diff.
- [ ] **Step 5: Document the new pieces.**
  - **`web-ui-system.md`:**
...
- [ ] **Step 6: Run the full gate.** `npm run lint`, `npm run lint:guards`, `npm run type-check`, `npm run test`, `npm run test:e2e:web`, `npm run test:visual:web` and `npm run docs:check`.
- [ ] **Step 7: Walk the keyboard** through both pages:
...
- **Layout:** the marketplace index overflows at 375, where the document is 442px wide. This predates PR 6 (found in PR 6's Task 6.19a check).
- **Also:**
