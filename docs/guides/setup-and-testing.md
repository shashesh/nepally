...
| `npm run test:security:listing-counters` | Listing counters are deduplicated (migrations 042, 043): a member adds at most one view per listing per day and one contact per listing ever, owners and inactive listings never count, and members cannot touch `listing_views` / `listing_contacts` |
