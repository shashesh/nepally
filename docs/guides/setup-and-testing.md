...
| `npm run test:security:listing-counters` | Listing counters are deduplicated (migration 042): a member adds at most one view per listing per day and one contact per listing ever, owners never count, and members cannot touch `listing_views` / `listing_contacts` |
