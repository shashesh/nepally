...
**The shared deletes don't trust that success.** `remove()` returns one row per object it deleted. `deletePostPhotos`, `deleteListingPhotos` and `deleteProfilePhoto` return an error unless that count matches the paths they asked for, so a missing file or a missing policy shows up as a failure instead of passing silently. The callers act on it:

- `removeProfilePhoto` deletes the file before it clears `users.profile_photo`. If the delete fails, the profile is left as it was, and the member sees an error and can try again.
- Post submit on web (`apps/web/src/lib/postSubmit.ts`) and mobile (`CreatePostScreen`) cleans up through the shared `cleanUpPostPhotos`. It deletes the photos just uploaded when the post write fails, and the photos the member dropped once an edit saves. The post write has already decided what the member sees, so a failed cleanup doesn't change it. Instead it logs `post_photos_cleanup_failed` with the paths that are still public.
