...
status: implemented
created: 2026-09-14
spec: docs/archive/specs/2026-09-14-web-ui-overhaul-design.md
---

# Web UI Overhaul Implementation Plan

> **Outcome (2026-09-25):** shipped as PRs #62–#66, #79–#81, #83, #85, #86, #88–#94 (PR 0 through 10c). The web app runs on the Ink & Marigold tokens and the Mantine component library, with no known serious or critical axe violations; mobile is on the shared logic the overhaul added; migration 040 is applied to `nusa-staging`. What was deliberately left is in **"Open follow-ups (after the overhaul)"** near the end of this file, and the next step is **"After the overhaul — Mantine 9"**.
...
**Spec:** [docs/archive/specs/2026-09-14-web-ui-overhaul-design.md](../specs/2026-09-14-web-ui-overhaul-design.md)
...
| 10b Web consistency + a11y | `fix/web-ui-polish` | 10b.1–10b.22, in four chunks | Merged (PR #93) | 2026-09-24 | branched from `master` at b544340 (PR #92 merged). **Chunk 1** (10b.1–10b.6, 3c813ab..7a7a07e, plus edf6ad3): gate green (web 1809, shared 737, mobile 636); review found nothing above LOW. Deviations: only the four not-found pages take `titleOrder={1}`; promote, moderation and search already have a `PageHeader` h1, so their empty states are h2s (edf6ad3); the feed's text-only stats take the buttons' padding; the chip line-height token is `--leading-tight`; `PostMeta` always renders its list (the scope badge is its last item); the sponsored card re-sets its accent `border-left-color` on hover; next/image 16's `priority` only preloads, so the hero photo also passes `fetchPriority="high"`; the other three `--shadow-float` rules are resting or floating elevation and stay. **Chunk 2** (10b.7–10b.12, 6c546d1..69456df): gate green (web 1845). `profile.page.tsx` 388 → 313 lines (panels in `components/profile/ProfileListPanels.tsx`), `users/[id].page.tsx` 316 → 283. Deviations: Manage Locations keys its focus arm on each row's id *and* default flag plus `showAdd` (ids alone never change on set-default); the add flow arms after its refresh; rows size their action slots with a local `--row-action-size` (the plan's `var(--ai-size, 28px)` would fail the guard). Review: one MEDIUM, fixed in 22ff454 with a test: `ProfilePhotoControl` keyed its arm on the photo URL, so a photo replaced while a removal was pending spent it; it keys on whether there is a photo. **Chunk 3** (10b.13–10b.18, 82bfcb9..c3c6c97, plus c2f8f58 making attendee avatars decorative): gate green (web 1857, shared 737, mobile 636). `AuthContext` calls `useRouter()`; the attendee dialog keeps its previous list visible while it reloads; listing detail's description got its own card, with DOM order top, aside, rest. Review: one MEDIUM, fixed in 2cba16b with a test that reproduced it (count 3 for 2): the bell's items ref caught up only after a render, so an INSERT for a row a load had just brought in was counted on top of the loaded count; the load now writes the ref as it lands. **Chunk 4** (10b.19–10b.21, 937b2f5..7a70a5c): gate green (web 1881, shared 751, mobile 636). `AddLocationForm`'s `onSave` returns `{ error?: string }`; a failed second page reads "Couldn't load more …"; the suite has no jest-dom, so the test-query rule says `getAttribute`. Also 2cba16b (`ErrorState` leaves out a message that only restates its title), ebc7401 (the last Mantine `data-*` and class assertions go; the public profile's Message button gains `aria-busy`), ad37e94. Review: one MEDIUM, fixed in 957ee5f: four marketplace page tests asserted only `ErrorState`'s fixed title, so they now also assert the RLS text never shows. **Finish** (10b.22): docs in 36c6745 (`web-ui-system.md`, the sign-up, marketplace and events feature docs, `INDEX.md`); e2e 159 → 161 (484616e: the attendee dialog reloads on reopen, the no-area marketplace). The first Docker `--update` rewrote 24 PNGs; reviewing them found two side effects, fixed in 36d3b52: the stats' padding inset the feed's row 8px, and `ScopeBadge` kept Mantine's line-height (30px beside 23px chips). The second run rewrote 23; desktop `marketplace` differed by noise only (max 2/255) and was restored, and the suite passed against it (39 passed, 1 skipped). 22 committed in d173ff2; `a11y-baseline.json` stays `{}`. Full gate green (lint, guards, type-check, `test`, e2e 161, visual 39, docs). The keyboard walk was not done by hand: focus behaviour is covered by the unit tests and e2e specs above. **Routed from 10b** (not new tasks): (1) **decided by the user, 2026-09-24:** posts, comments, search and listing detail keep showing full names as text (events and messages show public names); visible names are not shortened, and only avatar alt text follows decision 5; (2) shared-API network failures don't get the connection sentence, because those errors carry no status (shared-API change); (3) the public profile's About counts read 0 while its lists load or after one fails (pre-existing); (4) the public profile's error state has no h1, since `ErrorState` is an `Alert`; (5) a member with no metro still sees Create Listing, whose submit silently does nothing (pre-existing); (6) while a location row renames, Remove shifts into the pencil slot (pre-existing); (7) the last search tab is clipped at 375px with no scroll cue; (8) older fallback copy ("Failed to load your posts", `postSubmit`'s "Could not …") predates the "Couldn't …" style; (9) `Avatar.test.tsx` reads Mantine's `--avatar-radius`; (10) the disabled create-post button's `--surface-2` barely separates from the header. **Copilot review** round 1 (1 comment, real, pre-existing, dda02c2): a realtime INSERT accepted while a bell load was in flight was dropped when that load's older snapshot replaced the list and count, until the next poll. The bell now numbers the rows it accepts and each load merges back those that arrived after it started, counting only the unread ones its snapshot lacks; delete and mark-read update rows still waiting. Three regression tests (web 1884) |
| 10c Shared + mobile parity, archive | `chore/mobile-parity` | 10c.1–10c.16, in four chunks | Merged (PR #94) | 2026-09-25 | branched from `master` at 617bbb9 (PR #93 merged). Migration 040 (`full_name` ≤ 100) has the user's go-ahead (2026-09-24); applied in chunk 4, after 10c.5 makes `createUserProfile` fit it. **Chunk 1** (10c.1–10c.6, 490fc5b..fe43e96): gate green (shared 802, web 1877, mobile 636). `toApiError` covers 91 inline sites in 14 API files plus the 7 private helpers, detecting a fetch failure as `code === ''` with a message matching `/fetch\|network\|load failed\|abort/i` (checked against postgrest-js: only its fetch-rejection path yields `code: ''`). Deviations: the per-file fetch-failure tests live in one `api/apiErrors.test.ts`; six web tests mock shared `userMessage` to forward to their `logClientEvent` spy; mobile's local `AuthContext` `User` needed `phone?: string \| null` too; `createUserProfile` trims after the cut. Review found nothing above LOW. Routed: (1) those six mocks also log for real (console noise only); (2) mobile `AuthContext` keeps its own `User` type (pre-existing, against shared-first); (3) the code-point cut can split a ZWJ emoji sequence at character 100. **Chunk 2** (10c.7–10c.10, 0d8b246..f7f6749): gate green (mobile 666). Deviations: alert titles kept, only messages changed; new events `profile_update_failed`, `password_change_failed`; `EditProfileScreen.test` now mocks the real `NEPAL_DISTRICTS`. Routed: `ChangePasswordScreen` calls any re-auth failure "Current password is incorrect", outages included; mobile has no separate "couldn't process that image" copy; `SignupMethodScreen` detects a duplicate profile by message text; onboarding still logs with `console.error`; `EventDetailScreen:119`'s raw message goes to 10c.12. Chunk 2 review found nothing to fix; routed: the name field isn't written back after a save. **Chunk 3** (10c.11–10c.15, 62806cd..d14a932): gate green (shared 797, web 1877, mobile 698). `hooks/useMetroEventPages.ts` pages upcoming then past; `getEventsByMetro` deleted (99de324); `EventResponseButtons` replaced `RsvpButton`, with the blocked states as one disabled button ("Event Has Passed", "You're the Organizer", "Verify to RSVP"); the chunk's agent was stopped at its gate, which was then run by hand. Review: one HIGH, fixed in 9afcf04 with a test that reproduced it (4 for 5): a response that failed after a pull-to-refresh rolled back against the reloaded counts. **Chunk 4:** feature docs, schema doc and roadmap in 4691328; the open follow-ups are collected in "Open follow-ups (after the overhaul)" below. **Migration 040 is not applied**: the session's permission check stopped `apply_migration` on the shared staging project, so it waits for the user (follow-up 1). **Copilot review** round 1 (2 inline comments, one cause) answered 2026-09-25, real and fixed in 010ea17: mobile's chat thread and notifications screen read the clock only when their data changed, so Today stayed Today past midnight; both now take `now` from mobile's `useNow()`, as web's `MessageLog` and notifications page do, with a test each that advances the clock across midnight. Mobile 701. Round 2 (2 comments) answered 2026-09-25: (1) **its premise rejected, a real defect fixed**: it said mobile detail leaks the interested count on a private event, but private visibility hides names, not counts (`events.md`'s privacy rows), and both apps' list cards already show both counts on every event. On those lines a private event did show its going count twice, so the separate "N going" line goes and the one counts line shows for everyone (40595d1); (2) real: journey 13's Technical Requirements named `getUserRsvps` / `rsvpToEvent` / `unrsvpFromEvent`, which no screen calls; it now lists the response functions, with a dated note that its narrative still describes the single RSVP button (ad38d26). Round 3 (2 comments, both real) answered 2026-09-25: (1) React Navigation 7 updates a focused screen's params instead of pushing, so a notification or profile tap could move mounted mobile event detail to another event; the previous event's response refetch then replaced it and freed its controls, and the attendee list was reused. The view is now keyed on `eventId`, as web's is (f2d9a09); (2) both feeds kept the previous member's answers when `getUserEventResponses` failed. Each now records whose answers it holds, keeping the same member's on a failed refresh and clearing anyone else's; web's `useEventFeed` had the same gap from PR 7 (a1b78fa). Mobile 705; web's events tests 78; type-check and lint clean. Round 4 raised no findings. Merged 2026-09-25 as a9ba83f; **migration 040 applied to `nusa-staging` the same day** at the user's request (constraint read back validated; staging's longest name was 17) |
...
8. **Every preference control has a name** (recon 17). Each Mantine `Switch` takes a `label`, and its description sits outside the label, joined by `aria-describedby`. Mantine's own `description` renders inside the `<label>`, which made it part of the switch's name — found while building 9b.9. The two raw radio groups become `Radio.Group`s with a `label` ("Chat messages", "Likes") and a `description`. The Emergency alerts switch stays `checked disabled`, since it is always on, which the "can't have just used it" case allows. The fields sit in a `form`, and Save preferences is its submit button. Save preferences sets `aria-disabled` while it saves and shows a `Loader` in `leftSection`. A success calls `notify.success("Preferences saved.")`, which retires the "✓ Saved" label and its uncleared `setTimeout` (recon 18). A failure calls `notify.error("Couldn't save your preferences. Please try again.")` and leaves the edits in place.
...
- the form (decision 8), in two `section`s with `h2`s: "Delivery", then "Notification types". (The task first said "Push notifications", which repeated its only switch's label.)
...
- [ ] **Step 7: Walk the keyboard.** Keep the steps as e2e cases where they fit, as 8a did.
  ...
  - **Preferences:**
    ...
    - Enter on Save preferences submits the form, and focus stays on Save preferences while it saves. (Enter on a switch or radio does not submit, as in any browser.)
  - **Moderation:**
...
3. **An existing account goes to login**, whichever way Supabase reports it (recon 3). `isExistingAccountError` is true for the codes `user_already_exists` and `email_exists`. `signUpWithEmail` turns a returned user with `identities: []` into an `AuthError`-shaped `{ code: 'user_already_exists' }`. Signup then pushes `/login?reason=existing-account&email=…`, as it does today, and login's info alert stays. **Reversed at the user's call in Copilot's round 3** (2026-09-24): turning `identities: []` into an error made sign-up reveal who is registered, the very thing Supabase's look-alike answer exists to hide. A taken address now goes to verify-email like a new one, whose footer offers Log in (and Continue with Google). Only Supabase's explicit `user_already_exists` error, which comes only with email confirmation off and so hides nothing, still redirects to login.
...
## PR 10c — Shared + mobile parity, archive (`chore/mobile-parity`)

**Branch:** `chore/mobile-parity`, created from `master` at `617bbb9` (PR #93 merged). This PR is the one place the overhaul touches `apps/mobile/`: every mobile item the earlier PRs routed here, the shared pieces they need, migration 040, and archiving this plan.

**Recon** (2026-09-24, three read-only sweeps against `617bbb9`). What differs from the item list:

1. **Web has no `getErrorMessage` left**; 10b's `userMessage` replaced both copies. Mobile has three identical ones (`EditProfileScreen:38`, `ChangePasswordScreen:23`, `EmailSignupScreen:31`), each putting raw Supabase text on screen, plus inline copies in `SignupMethodScreen:82`, `EmailVerificationScreen:122,145` and `EventDetailScreen:119`.
2. **Mobile has no password reset at all**, so `requestPasswordReset` has one caller, web's `useProfileEditing.ts:125`. `ChangePasswordScreen` re-authenticates and calls `updateUser`.
3. **Shared-API network failures never read as connection failures** (10b's routed item 2). postgrest-js returns a fetch failure as a plain object (`code: ''`, message `TypeError: …`, `status: 0` on the response, not the error). ~70 sites in 10 API files turn any non-`Error` into `new Error('Failed to …')`, and 7 files have a private `toError`, so `isConnectionError` sees neither a status nor a name.
4. **`createUserProfile` inserts the provider's name unchecked** (web `authCallback.ts:48`, mobile `SignupMethodScreen:45` and `EmailVerificationScreen:106`). With migration 040, a Google name over 100 characters would fail sign-up. Staging: 24 users, longest name 17, none over 100, no existing `full_name` check.
5. **Mobile's photo upload doesn't copy `setProfilePhoto`**; it calls its two building blocks and hard-codes `500`. Mobile already has remove.
6. **`User.phone` is `phone?: string`**, so writing `null` needs the type widened. Nothing validates a member's phone on either app.
7. **Mobile's events list already has Interested / Going** (`EventsScreen:151–195` over `applyEventResponseChange`); only event detail is going-only (`isGoing: boolean`, one `RsvpButton`, no interested count). `hasUserRsvp` is gone.
8. **Mobile's `formatCount` differs above 10,000** (10.5K against shared's 10K) and carries the label; shared returns the number.
9. **Mobile's day labels differ from shared `formatDayLabel`**: neither shows a year; notifications uses the long month, finds yesterday by subtracting 24 hours (wrong across DST), and groups by the label string, so the same date in two years merges.
10. **Mobile chat shows full names everywhere**: `ConversationItem:95`, the thread header (`MessageThreadScreen:350`) and the block alert (`:259`). Route params carry `full_name` from six screens. `MessageThreadScreen` cleans up with `channel.unsubscribe()`, not `removeChannel`.
11. **Mobile tests replace `@nepally/shared` with an explicit object**, so every new shared import must be added to each affected test's mock.

**Decisions this breakdown locks in:**

1. **Shared errors keep their messages and gain a network flag.** Shared `utils/apiError.ts` exports `class ApiError extends Error { code?: string; status?: number }` and `toApiError(raw: unknown, fallback: string): Error`. An `Error` comes back unchanged; anything else becomes an `ApiError` whose message is **exactly what that site returns today** (the fallback, or the raw message where a private `toError` kept it), with the raw `code` copied, and `status: 0` when the raw is a PostgREST fetch failure (`code === ''` and a `message` naming a fetch or network failure). `isConnectionError` already treats `status === 0` as a connection failure. Every site in recon 3 and the 7 private helpers use it. No returned message changes, so no existing test changes.
2. **`userMessage` moves to shared** (`logic/userMessage.ts`, same signature) and web imports it from `@nepally/shared`; `apps/web/src/lib/userMessage.ts` and its test move with it. Mobile uses it wherever recon 1 found raw text, with `context: { platform: 'mobile' }`. **No shared `getErrorMessage`**: after 10b a raw-message helper is the thing we don't want.
3. **Mobile auth screens use `getAuthErrorMessage`**: `EmailSignupScreen` (`'log-in'` / `'sign-up'`), `SignupMethodScreen` (`'google'`), `EmailVerificationScreen`'s resend (`'resend'`). The OTP verify keeps `parseOtpError`'s three sentences. `ChangePasswordScreen` uses `userMessage` with "Couldn't change your password. Please try again."
4. **`requestPasswordReset(supabase, email, redirectTo): Promise<{ error?: Error }>`** lives in a new shared `api/auth.ts`.
5. **One name normaliser.** `validation/user.ts` exports `normalizeFullName(value)`, the transforms `fullNameSchema` already runs, which the schema now calls. `createUserProfile` stores `normalizeFullName(fullName)` cut to `FULL_NAME_MAX_LENGTH` code points, so a provider name always fits 040. Mobile edit profile and email sign-up validate with `fullNameSchema`, save its parsed value, and cap the input at `FULL_NAME_MAX_LENGTH`.
6. **Migration `040_user_full_name_length.sql`**: `ALTER TABLE users ADD CONSTRAINT users_full_name_length_check CHECK (char_length(full_name) <= 100)`, additive, the same form as `users_college_length_check`. It is applied to `nusa-staging` in chunk 4, after 10c.5 and the gate. The user gave the go-ahead on 2026-09-24.
7. **Phone clears to `null`.** `User.phone` becomes `phone?: string | null`; mobile saves `phone.trim() || null`.
8. **Mobile About You matches web**: seeded with `getAboutYouFormValues(user)`, typed `AboutYouFormValues`, limits from `COLLEGE_MAX_LENGTH` / `YEARS_IN_US_MIN` / `YEARS_IN_US_MAX`, parsed with `extendedProfileUpdateSchema` before saving, and the parsed values written back to the form.
9. **Mobile events page like web**: upcoming ascending, then past descending, each by its own offset through `getMetroEventsPage`, chaining the first past page when upcoming comes up short, one `now` per load, and stale pages dropped by a generation ref. The client-side filters stay. With no caller left, `getEventsByMetro` and its tests are deleted.
10. **Mobile event detail answers Interested or Going**: a new `EventResponseButtons` (two toggles with `accessibilityState.selected`; pressing the selected one clears it) replaces `RsvpButton`, which is deleted. It shows both counts and updates optimistically through `applyEventResponseChange` with rollback, as the list does. An unverified member still sees "Verify to RSVP".
11. **`formatCount` is shared's**: mobile's `EventCard` appends its label to shared `formatCount`, and "10.5K" becomes "10K" (web's rule).
12. **Day labels are shared's**: both mobile screens group by the local calendar date and label each group with `formatDayLabel(date, now)`. Notifications moves from the long month to web's short month, and gains a year for other years.
13. **Chat shows public names on mobile**: formatted where shown (`ConversationItem`, the thread header, the block alert), not in route params, so every caller is covered. `MessageThreadScreen` cleans up with `removeChannel`.
14. **Mobile's fixed realtime topics use `uniqueChannelTopic`**: the three in `HomeScreen` and the one in `NotificationsScreen`.

**How this PR runs.** Four chunks, as 10a and 10b. Each task writes its failing test first. At each chunk boundary run `type-check`, `lint`, `lint:guards` and the full unit suites of every touched workspace (web Vitest from a `C:\…` cwd; mobile Jest whenever shared changed), then one code-review agent over the chunk's diff. Fix CRITICAL and HIGH in one review commit; route the rest to the tracker. Mobile has no e2e run here (Maestro needs a device); the web e2e suite runs in chunk 4 because shared changed.

| Chunk | Tasks | Ends with |
|---|---|---|
| 1. Shared | 10c.1–10c.6 | `toApiError`, shared `userMessage`, `requestPasswordReset`, `formatMegabytes`, `normalizeFullName`, nullable phone, migration file |
| 2. Mobile profile + auth | 10c.7–10c.10 | Edit profile, About You, auth screens, sellers and megabytes |
| 3. Mobile events, realtime, chat | 10c.11–10c.15 | Events paging, event detail responses, counts, topics, day labels, public names |
| 4. Finish | 10c.16 | 040 applied, docs, roadmap, backlog, archive, draft PR |

## PR 10c — Task breakdown

Paths are under `packages/shared/src/` for shared and `apps/mobile/src/` for mobile unless given in full. Shared tests: `npm run test --workspace=packages/shared -- <paths>`. Mobile: `npm run test --workspace=apps/mobile -- <paths>` (add `--cacheDirectory <scratch>` if the default transform cache fails to write).

### Task 10c.1: `toApiError`
- [ ] Test `utils/apiError.test.ts`: an `Error` is returned as is; a plain object gets the fallback as its message and its `code`; a fetch-failure object gets `status: 0` and `isConnectionError` is true; a 4xx body does not.
- [ ] Adopt at every site in recon 3 and delegate the 7 private `toError` helpers, keeping each site's message (decision 1). One test per API file family is enough: a mocked fetch failure yields `isConnectionError(result.error) === true`.

### Task 10c.2: `userMessage` in shared
- [ ] Move `apps/web/src/lib/userMessage.ts` and its test to `packages/shared/src/logic/userMessage.ts`, export it from the barrels, and switch every web import to `@nepally/shared`. Add a case where a `toApiError` fetch failure yields `CONNECTION_ERROR_MESSAGE`.

### Task 10c.3: `requestPasswordReset`
- [ ] Shared `api/auth.ts` + test (calls `resetPasswordForEmail(email, { redirectTo })`, returns `{ error }` on a returned or thrown error). Web `useProfileEditing.ts` calls it; its tests mock the shared function.

### Task 10c.4: `formatMegabytes` in shared and web
- [ ] `validation/post.ts:53` and web `pages/posts/create.page.tsx:415` use `formatMegabytes(MAX_POST_PHOTO_BYTES)`; their copy is unchanged.

### Task 10c.5: `normalizeFullName`, `createUserProfile`, migration file
- [ ] Tests: `normalizeFullName` strips and collapses as the schema does; `fullNameSchema` still passes its suite; `createUserProfile` inserts a 150-character provider name cut to 100 code points (an emoji at the boundary is not split), and a bidi-laced name normalised.
- [ ] Write `supabase/migrations/040_user_full_name_length.sql` (decision 6). **Do not apply it in this task.**

### Task 10c.6: Nullable phone
- [ ] `types/user.ts` `phone?: string | null` and any update type that mirrors it; `type-check` across all workspaces.

### Task 10c.7: Mobile edit profile
- [ ] `screens/profile/EditProfileScreen.tsx`: photo through `setProfilePhoto` with `PROFILE_PHOTO_SIZE_PX`; name through `fullNameSchema` (error text from the schema, saved value parsed, `maxLength={FULL_NAME_MAX_LENGTH}`); phone `|| null`; `userMessage` replaces `getErrorMessage`, which is deleted. Tests cover each, including a cleared phone saving `null` and a raw RLS message never reaching `Alert`.

### Task 10c.8: Mobile About You
- [ ] `AboutYouSection.tsx` and `EditProfileScreen.tsx` per decision 8. Tests: a whitespace-only college saves `null`; a stored district outside `NEPAL_DISTRICTS` seeds as `null`; the college input's `maxLength` is `COLLEGE_MAX_LENGTH`; 100 years is refused.

### Task 10c.9: Mobile auth screens
- [ ] Decision 3 on `EmailSignupScreen`, `SignupMethodScreen`, `EmailVerificationScreen` and `ChangePasswordScreen`; delete the two remaining `getErrorMessage` copies. `EmailSignupScreen`'s sign-up validates the name with `fullNameSchema` and sends its parsed value. New tests for the three screens that have none (recon: `ChangePasswordScreen`, `EmailSignupScreen`), asserting the sentences, not Supabase's text.

### Task 10c.10: Sellers and megabytes on mobile
- [ ] `components/marketplace/ListingCard.tsx:23` and `ListingGridCard.tsx:37` call `isVerifiedSeller(listing)`; `screens/post/CreatePostScreen.tsx:804` uses `formatMegabytes`.

### Task 10c.11: Mobile events paging
- [ ] `screens/EventsScreen.tsx` per decision 9, ideally with the paging in a `hooks/useMetroEventPages.ts` of its own. Tests: upcoming first; a short upcoming page chains past; past arrives newest first; load-more follows the period's offset; a page from before a metro change is dropped. Delete `getEventsByMetro` and its tests.

### Task 10c.12: Mobile event detail responses and counts
- [ ] `components/events/EventResponseButtons.tsx` + test; `screens/EventDetailScreen.tsx` per decision 10 (an Interested member sees Interested pressed, not a "RSVP" button that upserts them to going); delete `RsvpButton` and its test. `components/events/EventCard.tsx` onto shared `formatCount` (decision 11).

### Task 10c.13: Mobile realtime topics
- [ ] `HomeScreen.tsx` (three channels) and `notifications/NotificationsScreen.tsx` topic names through `uniqueChannelTopic`; `chat/MessageThreadScreen.tsx` cleanup through `supabase.removeChannel`. Tests assert two mounts get distinct topics.

### Task 10c.14: Mobile day labels
- [ ] `MessageThreadScreen.tsx` and `NotificationsScreen.tsx` per decision 12; delete both local label functions. Tests: Today, Yesterday across a DST change, and a date last year showing its year in its own group.

### Task 10c.15: Mobile chat public names
- [ ] `components/chat/ConversationItem.tsx` and `MessageThreadScreen.tsx` (header, block alert, avatar label) show `formatPublicName` (decision 13). Tests: "Bikal Shrestha" reads "Bikal S." in the row, the header and the alert.

### Task 10c.16: Finish
- [ ] **Apply 040** to `nusa-staging` with `apply_migration`, then read the constraint back from `pg_constraint`.
- [ ] Docs: `docs/architecture/database-schema.md` (the new check), the feature docs for events, chat and profile where mobile behaviour changed, `docs/product/roadmap.md` (the overhaul and search shipped), `web-ui-system.md` if it names `lib/userMessage`.
- [ ] **Backlog.** 10b's routed items 3–10 go into this plan's closing section as open follow-ups, so they survive the archive.
- [ ] Full gate: `lint`, `lint:guards`, `type-check`, `test`, `test:e2e:web`, `docs:check`. No web visual page changes, so no re-baseline unless the suite fails.
- [ ] Tracker row → `In Review (PR #NN)`. Archiving (set this plan and the spec to `status: implemented`, `git mv` both to `docs/archive/`, `docs/INDEX.md`) is the PR's last commit, so the plan is still in `active/` while Copilot's review is answered — **do it once the review rounds are done, before the user marks it ready.**
- [ ] Push, open the draft PR with `.github/pull_request_template.md`, `gh pr edit <number> --add-reviewer @copilot`.

## Open follow-ups (after the overhaul)

Found during PRs 10b and 10c and deliberately not fixed there. None blocks launch; each is small. They outlive this plan's archive, so pick them up from here.

Also stale after the archive: eight code comments still cite `docs/specs/2026-09-14-web-ui-overhaul-design.md` at its old path (`tokens.css`, `Profile.module.css`, `PublicProfile.module.css`, `PublicProfileHeader.module.css`, `scrollingTabs.module.css`, `AccountDetails.module.css`, `AboutYouSection.module.css`, `scripts/guard-css-tokens.js`). They were left alone so the archive stayed a docs-only PR; point them at `docs/archive/specs/` in the next PR that touches those files. `037_search.sql` cites it too, but applied migrations stay as they are.

**Database**

1. ~~**Apply migration 040** (`users_full_name_length_check`) to `nusa-staging`.~~ **Done 2026-09-25** after PR #94 merged: the longest name was 17 characters, and `pg_constraint` reads back `CHECK ((char_length(full_name) <= 100))`, validated. A production database will need it too, with the rest of the migrations.

**Web** (routed from 10b)

2. The public profile's About counts read 0 while its lists load or after one fails (pre-existing).
3. The public profile's error state has no h1, since `ErrorState` is an `Alert`.
4. A member with no metro still sees Create Listing, whose submit silently does nothing (pre-existing).
5. While a Manage Locations row renames, Remove shifts into the pencil slot (pre-existing).
6. The last search tab is clipped at 375px with no scroll cue.
7. Older fallback copy ("Failed to load your posts", `postSubmit`'s "Could not …") predates the "Couldn't …" style.
8. `Avatar.test.tsx` reads Mantine's `--avatar-radius`, against decision 11's test-query rule.
9. The disabled create-post button's `--surface-2` barely separates from the header.
10. Six web tests (`useProfileEditing`, `postSubmit`, `profilePhoto`, `posts/[id]`, `profile`, `profile/locations`) mock shared `userMessage` to reach their `logClientEvent` spy, so the real logger also writes to the console in those failure tests. A test-only hook for shared's logger would remove the double.
11. The sidebar-card chrome (`EventAttendanceCard`, `EventOrganizerCard`, event detail's Manage card) becomes one primitive when a fourth copy appears.

**Mobile** (routed from 10c)

12. Mobile's `AuthContext` declares its own `User` interface rather than importing the shared type (pre-existing, against shared-first).
13. `ChangePasswordScreen` reports any re-authentication failure as "Current password is incorrect", outages included.
14. A photo that fails to resize or read shows the generic photo-update sentence; web has a separate "couldn't process that image" message.
15. `SignupMethodScreen` detects an existing profile by searching the error text for "duplicate" / "already exists"; the Postgres code `23505` is the reliable signal.
16. The onboarding screens log profile-creation and mark-verified failures with `console.error`, not `logClientEvent`.
17. After a save, edit profile's name field keeps what was typed rather than the normalised name that was stored (About You writes back; the name does not).
18. **Unverified:** mobile's other detail screens (post, listing, public profile) may share event detail's fault from PR 10c's Copilot round 3: React Navigation 7 updates a focused screen's params rather than pushing, so a screen that doesn't reset per id can show one item's state under another's id. Check each, and key the view on its id where needed.

**Shared**

19. `createUserProfile` cuts a provider name at 100 code points, which never splits a surrogate pair but can split a ZWJ emoji sequence at the boundary. `Intl.Segmenter` would cut on grapheme boundaries.
20. `fullNameSchema.max(100)` counts UTF-16 units while 040's `char_length` counts code points, so the form is stricter than the database for names with emoji. Harmless, but the two limits don't match.

**Product decisions still open**

21. What a paid promotion should do when its listing is inactive at payment time or is deactivated mid-run (a webhook check before activation, a refund, a pause) — from PR 8b.
