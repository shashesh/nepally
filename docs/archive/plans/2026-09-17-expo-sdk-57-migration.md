...
| expo-notifications: push APIs **throw** in Expo Go on Android (they only warned before) | 55 | Already handled. `registerForPushNotificationsAsync` returns early when `isExpoGo` (`apps/mobile/src/services/notifications.ts:30`). Importing the module only logs `console.warn`. **Wrong, see decision 9:** importing it throws on Android. |
...
1. **`configureForegroundNotificationHandler` is never called** (`apps/mobile/src/services/notifications.ts:73`), so notifications that arrive while the app is open are not shown. Wire it up in `App.tsx` and drop the deprecated `shouldShowAlert`. It is safe to call in Expo Go, where it returns before loading the module (decision 9).
...
12. **Make a lint rule enforce "no module-scope `expo-notifications` import"** (decision 9): `@typescript-eslint/no-restricted-imports` with `allowTypeImports: true`, switched off for `apps/mobile/src/services/notifications.ts`. Today the rule is only written down. Unit tests mock the module, so a regression shows up only on an Android phone in Expo Go. The change touches `eslint.config.mjs`, which the config-protection hook blocks, so it needs the user's explicit approval (decision 1).
13. **Real splash artwork.** `apps/mobile/assets/splash.png` is a 1×1 placeholder. The plugin draws it 200 wide on #1565C0, and `apps/mobile/assets/README.md` now describes that.
14. **The setup guide has no `npx playwright install chromium` step** (decision 6). CI installs browsers itself (`.github/workflows/ci-job.yml`), but local setup never does, and every Playwright upgrade needs a new browser build.
